...
export class ApiClientService {
  ...
  addLecture(name: string, course: string, exam: string): Observable<string> {
    this.authentificated = true;
    return this.http.post<string>(this.baseUrl.concat("/lectures"), {
      name: name,
      course: course,
      exam: exam
    });
  ...
  getLecture(lectureId: string): Observable<JSON> {
    return this.http.get<JSON>(
      this.baseUrl.concat("/lectures/").concat(lectureId)
    );
  }
